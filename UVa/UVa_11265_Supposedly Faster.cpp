...
//	freopen("11265.txt", "w", stdout);
	...
	while (scanf("%d", &n) == 1)
	{
		...
		x = lns_v[idx - 1];
		for ( ; it_l < sz_l - 1; it_l++) if (intersect(pt_l[it_l], pt_l[it_l + 1]).x >= x) break;
		for ( ; it_u < sz_u - 1; it_u++) if (intersect(pt_u[it_u], pt_u[it_u + 1]).x >= x) break;
